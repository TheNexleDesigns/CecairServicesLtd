...
(function(){
  ...
  function setupWelcome(){
    ...
    var src = isDesktop ? 'assets/welcome-desktop.mp4' : 'assets/welcome-mobile.mp4';
    ...
    document.querySelectorAll('.radio-chip').forEach(function(chip){
      chip.addEventListener('click', function(){
        ...
        if(input.name === 'contactNumber'){
          var panel = chip.closest('.form-panel');
          var num = input.value === 'secondary' ? WA.secondary : WA.primary;
          var callLink = panel.querySelector('.call-now-instead');
          if(callLink) callLink.setAttribute('href', 'tel:+' + num);
        }
      });
    });
  ...
  function submitForm(form){
    ...
    // Open WhatsApp right away — this IS the "send" action; the success
    // state below is just a fallback in case the popup gets blocked.
  ...
  function setupGallery(){
    ...
    GALLERY_SETS.forEach(function(set, idx){
      ...
      var videoCount = set.images.filter(function(html){ return /<video/i.test(html); }).length;
      var photoCount = set.images.length - videoCount;
      var parts = [];
      ...
      var countLabel = parts.join(' · ');
      var playIcon = videoCount ? '<span class="gi-play"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>' : '';
      item.innerHTML =
        '<div class="gi-media">'+set.cover+'</div>'+
        playIcon+
        '<span class="gi-count">'+countLabel+'</span>'+
